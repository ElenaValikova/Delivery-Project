...
const valid = function (str) {
  const nameReg = /^[a-zA-Z][a-zA-Z0-9-_\.]{1,20}$/;  // регулярные выражения
...
const openGoods = function (event) {
  ...
  if (login) {
    ...
    if (restaurant) {
      ...
      toggleModalAuth();    // пока не авторизовался-не можеш зайти (вылетает окно для автор.)
    }
  }
};
...
const init = function () {
  ...
  new Swiper('.swiper-container', {
    loop: true,  // бесконечно листался
    autoplay: true,
    slidesPerView: 1  // сколько слайдов в колонке
  });
};
